...
const selects = s.map((x) => x.selectMs).slice().sort((a, b) => a - b);
const p50 = selects[Math.floor(selects.length * 0.5)] ?? 0;
const p95 = selects[Math.floor(selects.length * 0.95)] ?? 0;
const maxAppearAfter = Math.max(0, ...s.filter((x) => x.t > 0).map((x) => x.appeared));
lines.push('');
...
lines.push(`p50 ${p50.toFixed(3)} ms   p95 ${p95.toFixed(3)} ms   max appear after t=0: ${maxAppearAfter}`);
lines.push('');
lines.push('t=0 appear=194 is the initial visible set, not a pop.');
lines.push('Samples > 1.0 ms on this host do not track visited-node count or pool misses; treat as GC unless a GPU HUD `select` disagrees.');
