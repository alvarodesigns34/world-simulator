...
## M1 scope reconciliation (2026-09-12)

Where M1 actually stands after Grok's red-team and this consolidation. Split by
**who can close it**, because that is the only distinction that matters now:
Astra's time is the scarce resource and she must not spend it on anything a test
could have settled.

### CLOSED TECHNICALLY — demonstrated by tests and CI

| M1 item | Evidence |
| --- | --- |
| WebGPU device, typed failure, no WebGL2 | `device.test.ts`; timestamp-query retry reads the device, not the adapter |
| Reversed-Z, infinite far plane | depth resolution measured across the altitude sweep, >100× finer than a pixel everywhere |
| Camera-relative f32 conversion | worst corner error < 1/1000 px at every altitude from 1 m to 40 000 km |
| PCF + quaternion camera, polar-safe | five polar-crossing tests; drag now uses qUp/qRight, which world-Z and (−y,x,0) cannot provide at ±Z |
| Cube-sphere: six faces outward, winding, seams | DEC-035 table; 12-edge weld test; ∂u×∂v outward on an interior grid per face |
| WGSL validity | parsed with a real grammar in CI, plus the 145-word reserved list |
| CPU↔GPU struct layout | reflected from the shader and asserted against the packer |
| Front-face convention | measured on the GPU at startup; no inconclusive path enables culling, proved over all 8 pixel combinations |
...
| Descent harness reaches 1 m | matches the criterion below; no regression |
| Simulation stands alone | `check:sim-standalone` in CI |

### OPEN, NON-VISUAL — closable without a GPU or Astra

| M1 item | Owner | Note |
| --- | --- | --- |
| T-0013 browser SAB / COOP-COEP table | Grok | 50 MB numbers are in; the tile-sized path in a real browser is not |
| T-0021 `stableMath` Tier-A transcendentals | Grok | R-11; Tier A is still a promise |
| T-0020 valence-3 corner topology for hydrology | Grok | Orientation is settled by DEC-035; the corner story is not |
| T-0065 graceful LOD degradation at the patch cap | Grok | Not hit at τ=4.0, so P2 |

### ASTRA-ONLY — needs a GPU or an eye, and nothing else will do

| M1 item | Why only Astra |
| --- | --- |
| Ampere canvas confirmation | Whether it draws at all on real hardware |
| Sphere completeness | Holes are a visual judgement even with the topology tested |
| Camera orientation and feel | "Is up up", does the drag feel right |
| Pole motion | The maths is proven; whether the *motion* reads as continuous is not |
| Popping as seen | No CDLOD morph yet — is hysteresis alone tolerable |
| Vertex swim | Arithmetic says sub-millimetre; unverified on a GPU (R-09, E2) |
| Frame pacing | CPU select is measured; the GPU column is empty |
| Horizon limb | ≤ 1.6 px of sag — visible or not |
| Scale perception | Planet or ball |
| E1: 17 / 33 / 65 on the GPU | Tessellation is geometrically inert, so 17×17 may simply win |

### FORMALLY DEFERRED TO M2

| Item | Decision |
| --- | --- |
| CDLOD morphing (T-0015) | Deferred; Astra's popping verdict decides whether it blocks |
| Spherical patch interpolation (T-0064) | Deferred; derivation in `RENDERING.md` §4.0b. M2 cannot skip it — displacing a bilinear quad does not give terrain on a sphere |
| Ancestor upsampling + chain prefetch (T-0052) | Needs tiles |
| Terrain, geology, ocean, atmosphere, climate, biosphere, cities | M2+ |

### ROADMAP DRIFT — resolved

| Drift | Resolution |
| --- | --- |
| Criterion said descent to **1 m**; the harness stopped at **2 m** | Harness lowered to 1 m (T-0077). It reaches it with no regression, so the criterion stands unamended. An acceptance criterion the harness does not reach is not an acceptance criterion. |
| τ = 2.0 px stated, ~4.0 px delivered | Resolved at DEC-032's amendment: the model was corrected and τ now states what it delivers |
| `maxVisiblePatches: 1200` vs τ = 2.0 px | Resolved by DEC-032: the budget is a function, and cannot produce that pair |
