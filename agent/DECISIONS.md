...
## DEC-054 — Filtered render corridors retain authoritative edge identity

**Date:** 2026-09-13
**Status:** Accepted
**Author:** ChatGPT (Integration / Runtime QA)

### Context

The app adapter intentionally does not draw sea or river routes as solid boxes.
It cached only the remaining land corridors, then read quality from
`network.edges[corridorIndex]`. Once a skipped water edge preceded a land edge,
those indices described different edges. A second mismatch came from mode:
investment can upgrade a road to rail without changing the physical corridor or
incrementing `topologyGeneration`, so caching the render kind with geometry left
the old road visible. Physical-route `bridgeCells` were calculated but never
reached the renderer.

### Decision

A derived corridor stores the index of the authoritative economic edge that
created it. Geometry stays cached by topology generation, while quality and mode
are refreshed from that exact edge each frame. Bridge cells are projected onto
the decimated corridor as deterministic segment indices and rendered as bridge
spans. Sea lanes remain undisplayed; only their real coastal outlet nodes appear.

The index is identity for a derived cache, not authority: the network continues
to own topology, route cells, mode and quality, and no renderer value is written
back to simulation state.

### Consequences

- Mixed sea/land edge ordering can no longer shift road/rail quality.
- Road-to-rail upgrades change presentation without forcing route regeneration.
- Physical infrastructure bridges are visible only where the authoritative route
  identifies a river crossing.
- Timeline rewind/live restore invalidates or refreshes the derived scene and is
  checked by exact instance-byte comparison.
